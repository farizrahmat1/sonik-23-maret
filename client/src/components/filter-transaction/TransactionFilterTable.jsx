...
export function TransactionFilterTable() {
  ...
  useEffect(() => {
    const fetchDataTransaction = async () => {
      try {
        ...
        const filterMonth = res1.data.filter(
          (item) =>
            new Date(item.stop_timestamp).getMonth() == new Date().getMonth()
        );
        const dataFilter = filterMonth.sort((a, b) =>
          a.transaction_pk > b.transaction_pk ? 1 : -1
        );
        setTransactionStart(dataFilter);
        // setTransactionStart(res1.data);
      } catch (err) {
        console.log(err);
      }
      ...
      <TableContainer component={Paper}>
        ...
        <button className="buttonChart">
          <CSVLink
            className="CSVButton"
            data={dataDownload}
            filename={`transaction-filtered (${monthDate})`}
          >
                  ...
                  <React.Fragment>
                    <StyledTableRow>
                      <TableCell
                        component="th"
                        scope="row"
                        key={filteredDataTransaction.ID}
                      >
                        {index + 1}
                      </TableCell>
                      <TableCell align="center">
                        {filteredDataTransaction[index].NAME}
                      </TableCell>
                      <TableCell align="center">
                        {filteredDataTransaction[index].connector_name}
                      </TableCell>
                      <TableCell align="center">
                        {filteredDataTransaction[index].transaction_pk}
                      </TableCell>
                      <TableCell align="center">
                        {filteredDataTransaction[index].id_tag}
                      </TableCell>
                      <TableCell align="center">
                        {moment(
                          filteredDataTransaction[index].start_timestamp
                        ).format("MMMM Do YYYY, h:mm:ss a")}
                      </TableCell>
                      <TableCell align="center">
                        {filteredDataTransaction[index].start_value}
                      </TableCell>
                      <TableCell align="center">
                        {moment(
                          filteredDataTransaction[index].stop_timestamp
                        ).format("MMMM Do YYYY, h:mm:ss a")}
                      </TableCell>
                      <TableCell align="center">
                        {filteredDataTransaction[index].stop_value}
                      </TableCell>
                      <TableCell align="center">
                        {filteredDataTransaction[index].stop_value -
                          filteredDataTransaction[index].start_value}
